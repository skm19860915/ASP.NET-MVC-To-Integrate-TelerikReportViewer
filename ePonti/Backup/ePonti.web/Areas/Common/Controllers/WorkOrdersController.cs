...
{
    public class WorkOrdersController : web.Controllers._baseMVCController
    {
        ...
        public ActionResult UpdateActuals(int WoID, List<int> AssignmentIDs, DateTime? ActualDate, decimal? ActualHours, double? PercentCompleted, int WoTypeID = (int)EnumWrapper.WorkOrderTypes.Scheduled)
        {
            ...
            {
                bool status = repo.UpdateWorkOrderActuals(siteusercompanyid, AssignmentIDs, ActualDate, ActualHours, PercentCompleted, (EnumWrapper.WorkOrderTypes)WoTypeID, ConfigurationManager.ConnectionStrings["DefaultConnection"].ToString());
                //bool status =rep
                if (status)
                {
                    if (WoTypeID == (int)EnumWrapper.WorkOrderTypes.Punchlist)
        ...
        public ActionResult ReportViewer(int? WoID)
        {
            if (WoID == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            ...
            RptCoHeaderBySiteCoID_Result comInfo = db.RptCoHeaderBySiteCoID(siteCoID).FirstOrDefault();
            ViewBag.WoInfo = db.RptWoInfoByWoID(WoID).FirstOrDefault();
            ViewBag.WoItems = db.RptWoTasksByWoID(WoID).ToList();
            ViewBag.WoID = WoID.Value;
            return View(comInfo);
        }
        public ActionResult Report(int? WoID)
        {
            if (WoID == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
        ...
        public ActionResult SaveAsPdf(int? WoID)
        {
            if (WoID == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            ...
            return new ActionAsPdf("ReportViewer", new { WoID = WoID })
            {
                FileName = "WO_"+ WoID + ".pdf",
                Cookies = cookies
            };
        }
    }
