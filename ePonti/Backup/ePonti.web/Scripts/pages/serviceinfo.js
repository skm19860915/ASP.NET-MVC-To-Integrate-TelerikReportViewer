...
function getContactData() {
    $.get(_getContactDataUrl, { ContactID: $('#ClientID').val() }, function (res) {
        if (res) {
            ...
            $('#ServicePhone').val(res.phone && res.phone.Phone);
...
$(function () {
    ...
    $('#open-edit-popup').on('click', function () {
        $('#editableArea').load(_serviceEditViewUrl, function () {
            $('#StartDate').val($('#StartDate').attr('data-value'));
            $('.date-picker').removeAttr("data-val-date");
            initDatePicker();
            onEditPopupOpened();
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res.status === 'error') {
                        ...
                        $alert.html('Service saved.');
                        showAlert($alert, 'success');
                        //var url = _serviceDetailUrl.replace("SERVICEID",res.serviceid);


                       // var url = "WorkOrders/Edit"

                        //if (isNew) {
                        //    url = _quoteDetailUrl + res.quoteid;
                        //}
                        document.location = res.WO_URL;
                    }
                }
            });
        }
    ...
    (function () {
        //if new service page, then also load contact info
    ...
    $('body').on('click', '.stages', function () {
        ...
        $.get(_getStageDetailsUrl, { ProjectID: $('#ServiceID').val(), StageID: $(this).find('.item-id').val() }, function (res) {
    ...
    (function () {
        ...
        $('body').on('click', additionalTab + ' .update-item', function () {
            ...
            var data = { ItemID: itemID, ProjectID: $('#ServiceID').val() };
        ...
        $('body').on('click', additionalTab + ' .item-row .delete', function () {
            ...
            $.post($tab.data('delete'), { ItemID: itemID, ProjectID: $('#ServiceID').val() }, function (res) {
                $row.remove();
...
(function () {
    ...
    $('.save-project-contact').on('click', function () {
        ...
        $.post(_addServiceContactUrl,
            { ServiceID: $('#ServiceID').val(), ContactID: $optionContact.val(), RelationshipID: $optionRel.val() },
            function (res) {
...
$(function () {
    ...
    $('.parts-por, .parts-delivery, .parts-assign').on('click', function () {
        if ($('tr.part input.item-id:checked').length > 0) {
    ...
    $('.parts-refresh').on('click', function () {
        ...
        $.get(_getServicePartsUrl, { ProjectID: $('#ServiceID').val() }, function (res) {
            if (res) {
...
$(function () {
    ...
    $('body').on('click', '.grids .item-row .save', function (e) {
        ...
        dataToPost['ProjectID'] = $('#ServiceID').val();
